...
const urlShortner = async (req, res) => {
  try {
    ...
    if (!longUrl) {
      return res
        .status(400)
        .send({ status: false, msg: "Please provide a longUrl into postman" });
    }
    if (
      ...
      return res
        .status(400)
        .send({ status: false, msg: "Please provide a valid longUrl" });
    }
    ...
    if (duplicateLongUrlCache)
      return res
        .status(302)
        .send({
          msg: "Already a shortUrl exist with this Url in Cache",
          urlDetails: duplicateLongUrlCache,
        });
    ...
    if (duplicateLongUrlDB)
      return res
        .status(302)
        .send({
          msg: "Already a shortUrl exist with this Url in DB",
          urlDetails: duplicateLongUrlDB,
        });
  ...
  try {
    ...
    if (urlFromCache) {
      ...
      if (urlFromMongoDB) {
        ...
        return res
          .status(404)
          .send({ status: false, msg: "No url found with this urlCode" });
      }
    }
  } catch (err) {
    console.log(error);
